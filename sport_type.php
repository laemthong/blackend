...
<div class="container">
    ...
    if ($result->num_rows > 0) {
        ...
        if ($row["status"] == "active") {
            echo "<a class='btn btn-suspend' href='sport_type.php?suspend=".htmlspecialchars($row["type_id"])."' onclick='return confirmSuspend()'>ระงับ</a>";
        } else {
            echo "<a class='btn btn-reactivate' href='sport_type.php?reactivate=".htmlspecialchars($row["type_id"])."' onclick='return confirmReactivate()'>เปิดใช้งาน</a>";
        }
    ...
    function confirmDelete() {
        if (confirm("คุณแน่ใจว่าต้องการลบนี้หรือไม่?")) {
    ...
    function editType(type_id, type_name) {
        ...
        // แสดงข้อความแจ้งเตือนผู้ใช้ว่าเตรียมแก้ไขข้อมูล
        alert("กรุณาแก้ไขข้อมูลในฟอร์มและกดบันทึก");
    }   
    </script>
